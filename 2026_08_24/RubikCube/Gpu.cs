...
namespace RubikCube
{
    ...
    public static unsafe class Gpu
    {
        ...
        public static int SeedPerturbPercent = 0;   // GPU seeder: % chance per step to take a NON-SEATING progress move
                                                    // (4-cycle -> 2+2 split) instead of seating. 0 = pure seating (baseline A/B arm).
        ...
        public static void BuildShaders()
        {
            ...
            CompileComputeProgram(SeedMovesProgram, "Resources.SeedMoves.glsl.c");
        ...
        // random SEATING descent of the active cubie's orientation (Resources/SeedMoves.glsl.c). Replaces the host
        ...
        static void DispatchSeedMoves(TRubikSolver solver)
        {
            ...
            int target = TGA<TRubikGenome>.PopulationCount * GetDefineValue("SEED_RATIO") / 100;
            if (target <= 0)
            {
                NumSeeds = 0;
            ...
            uint startM = solver.SeedStartM(out int repIndex, out uint realM, out uint collAxis);  // descent: active cubie; coherence: X = M.Dᵀ
            ...
            OpenGL.Uniform1ui(1, (uint)repIndex);                       // activeIndex (rep, for the collateral slice)
            OpenGL.Uniform1ui(9, startM);                               // startM (relative X to decompose)
            OpenGL.Uniform1ui(10, realM);                              // realM (rep's real orientation, for the slice)
            OpenGL.Uniform1ui(11, collAxis);                           // preferred collateral axis (keep rep's block together)
            OpenGL.Uniform1ui(12, (uint)SeedPerturbPercent);          // non-seating perturbation rate (0 = pure seating)
        ...
        static float EvalZeroSpecimen(TRubikSolver solver, out uint structure, out uint ladder)
        {
            ...
            OpenGL.MemoryBarrier(OpenGL.MemoryBarrierFlags.ShaderStorage);
            var result = new int[SpecimenInts];                    // read the WHOLE specimen so we can also grab Structure
        ...
        public static TRubikGenome ExecuteGA(TRubikSolver solver, out int gens)
        {
            ...
            best.Fitness = float.MaxValue;   // running-best sentinel: the first generation's winner always beats it
            ...
            fixed (int* bestBufPtr = bestBuffer)
            {
                ...
                for (; generationNo < TGA<TRubikGenome>.GenerationsCount; generationNo++)
                {
                    ...
                    // 3. Read this generation's best (Population[0] after the sort), track the running best on
                    //    the CPU, and early-exit as soon as it beats the current accepted Score.
