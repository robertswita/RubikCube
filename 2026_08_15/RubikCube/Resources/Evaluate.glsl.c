...
void main()
{
		...
		for (int m = 0; m < GENES_COUNT; m++)
		{
				...
				uint NORM = 5 * N - 10 + N;// 1u * (FLOOR - 1u + uint(N - findMSB(FLOOR)));   // floor value of 2*P at all-singletons FLOOR
				...
				for (uint i = 0; i < countActive; i++) {
						...
						if (cur != ClusterCubies[i]) changed = true;   // did this prefix move the active cluster?
				...
				if (!changed) local_fA_sum += 2 * CUBIES_COUNT;   // ~0,2 -> ~32,2
				uint ladder = uint(local_fA_sum * float(NORM)); // integer structure value for THIS step (0 = solved/descent, the best possible)
...
//            if (COHERENT != 0u && scrambled > 0u)
				...
				//if (scrambled > 0u && changed && scrambled <= G / 2) {
						...
						//		if (ARR_MASK_TEST(stateDone, gi)) continue;   // state already decomposed at an earlier cubie
						...
						//				if (local_active[scr[k]] == sa) { grp[gn++] = scr[k]; ARR_MASK_SET(stateDone, k); }
						...
						//						uint id = ActivePos[grp[k]];           // grp holds active-index; resolve to cubie id for curCoord
						//						bool inBox = true;
						//						for (uint j = 0u; j < uint(N); j++)
						//								if (MASK_TEST(bMask, j) && curCoord(sa, id, j) != curCoord(sa, refId, j)) { inBox = false; break; }
						//						if (!inBox) continue;
						//						cnt++;
						//						for (uint j = 0u; j < uint(N); j++) MASK_SET(valMask[j], curCoord(sa, id, j));
						//				}
						...
						//						dHist[agreeAxes] += 1u; pieces += 1u;   // count complete blocks -> gateway = a SINGLE block (pieces == 1), the orient identity-collapse trigger
						...
						//								uint id = ActivePos[ai];       // cubie id for curCoord
						//								bool inBox = true;
						//								for (uint j = 0u; j < uint(N); j++)
						//										if (MASK_TEST(bMask, j) && curCoord(sa, id, j) != curCoord(sa, refId, j)) { inBox = false; break; }
						//								if (!inBox) continue;
						//								uint v = curCoord(sa, id, a);
						//								if (!MASK_TEST(seenVals, v) && sp < 2 * N * N) {
						//										MASK_SET(seenVals, v);
						//										//MASK_SET(stMask[sp], a);
						...
						//uint scr[MAX_CLUSTER];
						//uint ns = 0u;
						//for (uint i = 0u; i < countActive; i++)
						//		if (local_active[i] != IDENTITY) scr[ns++] = i;
						...
						for (uint gi = 0u; gi < countActive; gi++) {
								uint sa = local_active[gi]; // state via active-index (handle each STATE once, at its first cubie)
								if (sa == IDENTITY || ARR_MASK_TEST(stateDone, gi)) continue; // state already decomposed at an earlier cubie
								// GENERALISED decomposition (ANY SIZE): split { cubies with state sa } into maximal COMPLETE
								// combinatorial boxes. A box fixes some axes to a full coord VALUE (0..SIZE-1), NOT a bit; branching
								// per axis = the DISTINCT values present (up to 2N), not 2 -- a 2N-ary tree, not binary. Box =
								// (fixedMask, refCubie): refCubie's coords give the fixed values (avoids packing arbitrary SIZE).
								...
								for (int guard = 0; guard < 4 * MAX_CLUSTER && sp > 0; guard++) {
										...
										for (uint k = 0u; k < countActive; k++) {
												if (local_active[k] != sa) continue;
												ARR_MASK_SET(stateDone, k);

												uint id = ActivePos[k];
												bool inBox = true;
												for (uint j = 0u; j < uint(N); j++) {
														if (MASK_TEST(bMask, j) && curCoord(sa, id, j) != curCoord(sa, refId, j)) {
																inBox = false;
																break;
														}
												}
										...
										uint agreeAxes = 0u;
										for (uint j = 0u; j < uint(N); j++)
												if (bitCount(valMask[j]) == 1u) agreeAxes++;

										// COMPLETENESS read DIRECTLY off the cluster: the block is coherent iff NO cubie OUTSIDE state
										// sa sits at the box's fixed coords -- then every position of the sub-orbit (fixed = Vf) is held by
										// an sa cubie, which is EXACTLY the old cnt == sub-orbit-size, but with no orbit-size combinatorics
										...
										bool complete = true;
										for (uint k = 0u; k < countActive; k++) {
												if (local_active[k] == sa) continue;                 // our cubies never break the block
												bool atBox = true;
												for (uint j = 0u; j < uint(N); j++)
														if (bitCount(valMask[j]) == 1u && curCoord(local_active[k], ActivePos[k], j) != curCoord(sa, refId, j)) { atBox = false; break; }   // AGREED axes (valMask), not just bMask: the block's sub-orbit is fixed on every axis the sa-cubies share
												if (atBox) { complete = false; break; }              // a non-sa cubie on a box position -> incomplete
										}
										...
										if (complete) {
												...
												// Iteracja po scr z filtrem stanu sa (zamiast grp)
												for (uint k = 0u; k < countActive; k++) {
														if (local_active[k] != sa) continue;
						...
						//uint domM = local_active[scr[0]]; uint domCount = 0u;
						//for (uint oi = 0u; oi < ns; oi++) {
						//		uint oc = 0u;
						//		for (uint oj = 0u; oj < ns; oj++) if (local_active[scr[oj]] == local_active[scr[oi]]) oc++;
						//		if (oc > domCount) { domCount = oc; domM = local_active[scr[oi]]; }
						//}
						//uint orientRaw = 0u;
						//for (uint oi = 0u; oi < ns; oi++)
						//		orientRaw += (pieces == 1) ? cubieL1(local_active[scr[oi]])       // uniform -> reference IDENTITY (collapse)
						//		                              : matL1(local_active[scr[oi]], domM);  // building -> reference DOMINANT (uniformise)
						//float orient = float(orientRaw) / float(countActive * MAX_CUBIE_L1 + 1);   // sub-unit (< 1): rides under 2*ladder
						current_step_fitness = float(2 * ladder + local_fA_sum) / float(NORM);   // selFitness: 2*ladder DOMINATES (structure ladder, the accept compares it); orient (< 1) is the sub-unit endgame orientation gradient (dominant-ref while building, identity-ref once uniform). ACCEPT is unchanged (Ladder-based) -- orient shapes only GA SELECTION.
