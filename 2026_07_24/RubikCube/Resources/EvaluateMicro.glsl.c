...
void main()
{
    ...
    for (int m = 0; m < GENES_COUNT; m++)
    {
        ...
        // compile flag: the host keeps it 0 through the count-peel descent and sets it to 1 once the residual
        // reaches the floor, so coherence shapes ONLY the endgame; below the floor the outer *FLOOR/scrambled
        // cancels the base's count, leaving fitness ~ suma -- the "PAIR EQUAL" ladder where k+0 and k+k tie.
