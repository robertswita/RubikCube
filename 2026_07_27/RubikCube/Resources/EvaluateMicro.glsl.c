    ...
    for (int m = 0; m < GENES_COUNT; m++)
    {
        ...
        if (COHERENT != 0u && scrambled > 0u && scrambled <= G) {   // latch OFF on the descent, ON at the floor
            ...
            for (int d = N; d >= 1; d--) {
                ...
                P2 += par * uint(d);                          // weight (depth+1) = (d-1)+1 = d
                E2 += (2u * par - occ) * uint(d);             // empty partners at depth d (=1 iff occ odd), depth-weighted
                occ = dHist[d - 1] + par;                     // dHist[0] stays 0 (no whole-cube block in the residual)
            }
            local_fA_sum *= float(P2 * S) / float((uint(N) * (uint(N) + 3u) / 2u) * scrambled);   // P2 * S, count-cancelled (floor P2 = N(N+3)/2)
