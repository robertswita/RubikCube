...
namespace RubikCube
{
    public class TRubikGenome : TChromosome
    {
        ...
        public unsafe static void ExecuteGA()
        {
            ...
            var bestBuffer = new uint[TGLContext.ResultBuffer.Size / sizeof(uint)];
            fixed (uint* bestBufPtr = bestBuffer)
            {
                ...
                var numGroupsX = TGA<TRubikGenome>.PopulationCount / 64;
                OpenGL.DispatchCompute(numGroupsX, 1, 1);
                OpenGL.MemoryBarrier(OpenGL.MemoryBarrierFlags.ShaderStorageBarrierBit);
                var currentFitness = ulong.MaxValue;
                for (int generation = 0; generation < TGLContext.GenerationsCount; generation++)
                {
                    var buf = TGLContext.ResultBuffer;
                    // reset bestFitness
                    OpenGL.BindBuffer(buf.Type, buf.Id);
                    // Nadpisujemy tylko pierwsze 4 bajty bufora (offset = 0)
                    OpenGL.BufferSubData(buf.Type, (IntPtr)0, 4, ref resetValue);

                    // 1. Ocena populacji
                    OpenGL.UseProgram(TGLContext.EvaluateProgram);
                    ...
                    if (best.Fitness < RubikCube.Score)
                        ...
                        OpenGL.UseProgram(crossoverShader);
                    OpenGL.Uniform1ui(0, timeSeed++);
                    OpenGL.DispatchCompute(numGroupsX, 1, 1);
                    OpenGL.MemoryBarrier(OpenGL.MemoryBarrierFlags.ShaderStorageBarrierBit);

                    // 3. Mutacja
                    OpenGL.UseProgram(mutationShader);
                    OpenGL.Uniform1ui(0, timeSeed++);
                    OpenGL.DispatchCompute(numGroupsX, 1, 1);
                    OpenGL.MemoryBarrier(OpenGL.MemoryBarrierFlags.ShaderStorageBarrierBit);
                }
