...
//            if (COHERENT != 0u && scrambled > 0u)
            ...
            for (uint gi = 0u; gi < ns; gi++) {
                ...
                uint grp[MAX_CLUSTER]; uint gn = 0u;          // members of THIS state, collected ONCE -- the DFS scans grp (O(g)/pop) instead of re-filtering scr (O(ns)/pop)
                for (uint k = 0u; k < ns; k++)
                    if (local_cubies[scr[k]] == sa) { grp[gn++] = scr[k]; stateDone[k >> 5u] |= (1u << (k & 31u)); }
