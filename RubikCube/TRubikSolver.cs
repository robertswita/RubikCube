...
namespace RubikCube
{
    ...
    public class TRubikSolver
    {
        ...
        float ScoreLadder;              // ladder value of the accepted config -- the coherence accept baseline (GPU-computed)
        ...
        public List<TMove> GetNextMoves()
        {
            ...
            if (candidate.Fitness <= Score)// || candidate.Fitness == Score && Stall >= TGA<TRubikGenome>.StallLimit)
            {
                ...
                Stall = 0;
                Score = Best.Fitness;
                ScoreLadder = Best.Ladder;   // advance the coherent structure baseline
        ...
        public TCubie RelativeSource(TCubie source, TMatrix D)
        {
            var c = source.Copy();
            if (D != null)
            {
        ...
        // Return the unpacked partner-group orientation D (as TAffine, or null when descent).
        // Caller computes startM via RelativeSource(ActiveCubie, D.M).Transform.OrthoPack().
        public uint SeedStartM(out int targetCubieId, out uint collAxis)
        {
            targetCubieId = ActiveCubie.Id;
            TCubie source = ActiveCubie;
            TAffine X = source.Transform.Clone(); // default (descent / single group -> solve)
            ...
            if (CoherenceRetarget && Coherent != 0)
            {
                ...
                if (atMin.Count > 0)
                {
                    ...
                    if (atMin.Count >= 2)                            // >=2 lowest groups -> pair (random partner)
                    {
                        ...
                        var DT = g2[0].Transform.M.Transpose() * (1f / TCubie.Scaling[0]);                       // D = partner group's orientation (build k+k)
                        X.M = source.Transform.M * DT;
                        var c = source.Copy();
                        c.Transform.Origin = DT * c.Transform.Origin;
                        targetCubieId = TCubie.DimsSizes.CoordsToBitIndex(c.Position);
                    }
                }
            }
