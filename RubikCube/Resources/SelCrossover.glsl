...
void main() {
    ...
    uint momIdx = randNext(winnerCount);
    uint dadIdx = (momIdx + 1u + randNext(winnerCount - 1u)) % winnerCount;

    // Split point in the first half. Mirrors CPU: Rnd.Next(GENES_COUNT) / 2.
    uint startPos = randNext(GENES_COUNT) / 2u;
    uint stopPos = startPos + 1u + randNext(uint(N - 1));
    ...
    //}
    if (numSeeds > 0u && randNext(100u) < uint(MUTATION_RATIO))
    {
        uint sbase = (randNext(numSeeds) * SEED_STRIDE);
        uint g = 0;
        for (; g < SEED_STRIDE; g++)
        {
    ...
    //        child[g] = FreeMoves[randValue % uint(FreeMoves.length())];
    //    }
    //}
