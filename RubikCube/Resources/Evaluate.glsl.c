...
void main()
{
		...
		for (uint w = 0u; w < (GENES_COUNT + 31u) / 32u; w++) solvedBroken[w] = 0u;
		for (uint s = 0u; s < countSolved; s++) {
				//uint id = SolvedCubies[s];
				uint mat = IDENTITY;// Cubies[id];
				for (uint g = 0u; g < uint(GENES_COUNT); g++) {
						mat = TurnSingleCubie(mat, SolvedPos[s], getMove(Population[specimenID].Moves[g]));
						if (cubieL1(mat) != 0u) solvedBroken[g >> 5u] |= 1u << (g & 31u);
				}
		}
		...
		for (int m = 0; m < GENES_COUNT; m++)
		{
				...
				uint NORM = 1u * (FLOOR - 1u + uint(N - findMSB(FLOOR)));   // floor value of 2*P at all-singletons FLOOR
				NORM *= countActive + 1u;
				//if (COHERENT == 0u)
				//{
				bool changed = false;
...
//            if (COHERENT != 0u && scrambled > 0u)
				...
				float brk = float((solvedBroken[uint(m) >> 5u] >> (uint(m) & 31u)) & 1u);   // any Solved broken at this prefix?
