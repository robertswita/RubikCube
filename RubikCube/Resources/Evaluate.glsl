...
// The coherence fine measure aims cubies at the SAME orientation the seeds target: the block orientation D (packed,
// == SeedStartM's D) while building, IDENTITY once one block remains. Host sets it per GA run (Gpu.TargetD).
layout(location = 13) uniform uint targetD;
...
uint matL1(uint M, uint R)
{
    ...
    for (uint r = 0u; r < N; r++)
    {
        ...
        uint d = (rowM > rowR) ? (rowM - rowR) : (rowR - rowM);
        dist |= d << (r * BITS_PER_ROW);
        //dist += d;
    }
    return dist;
    //return (M > R) ? (M - R) : (R - M);
...
void main()
{
    ...
    if (specimenID >= Population.length())
        return;
    ...
    float max_fA = maxClusterState * (float(countActive) + 1.0);
		...
		//uint LADK = 1u << (N - 1u); LADK = LADK * LADK;   // G*G > max bCount (crossed same-size block pairs)
    ...
    float specimen_best_ladder = 0u; // ACCEPT metric: integer structure value at the best step (Setup: Ladder)
    ...
    for (int w = 0; w < solvedBroken.length(); w++)
        solvedBroken[w] = 0u;
    ...
    for (int m = 0; m < GENES_COUNT; m++)
    {
        ...
        uint structure = 0u; // piece histogram for THIS step (0 = not decomposed)
				...
				//uint NORM = 2u * (FLOOR - 1u) + 4u * (N - uint(findMSB(FLOOR)));
        ...
        for (uint i = 0; i < countActive; i++)
        {
            ...
            if (cur != ClusterCubies[i])
                isNoOp = false; // did this prefix move the active cluster?
        ...
        float ladder = scrambled == 0u ? 0.0 : 1.0 * float(NORM); // integer structure value for THIS step (0 = solved/descent, the best possible)
        ...
        if (isNoOp)
            local_fA_sum += 2 * CUBIES_COUNT; // ~0,2 -> ~32,2
				...
				//else
				//{
        uint G = 1u << (N - 1u); // gateway 2^(N-1); the endgame TARGET is G+G (scrambled = 2G)
...
//            if (COHERENT != 0u && scrambled > 0u)
        ...
        //{
        //    if (local_active[oi] == IDENTITY)
        //        continue;
        //    orientRaw += matL1(local_active[oi], targetD); // building  -> targetD (the seeds' block orientation)
        //    //orientScr++;
        //}
        //float orient = float(orientRaw) / float(countActive * uint(MAX_CUBIE_L1) + 1u); // sub-unit (< 1)
        //if (orientScr > 0)
        //    orient *= float(FLOOR) / floor(orientScr);
        //float current_step_fitness = COHERENT != 0u ? float(ladder) / float(NORM) : local_fA_sum;
        float current_step_fitness = local_fA_sum;
        if (COHERENT != 0u && !isNoOp && scrambled > 0u && scrambled <= G)
        { // UNIFIED: decompose EVERY residual for its REAL structure -- an isolated singleton (mono-twist) gets its own EXPENSIVE ladder (deepest structure), so the structure-dominant additive below deters isolating the last cubie. No lie, no even/odd split. Full-cluster aMax==0 trap walled below.
            uint aMax = 0u;
            uint aSum = 0u;
            uint seenAgree = 0u;
            uint pieces = 0u;
            ...
            for (int di = 0; di < stateDone.length(); di++)
                stateDone[di] = 0u;
            ...
            for (uint gi = 0u; gi < countActive; gi++)
            {
                ...
                if (state == IDENTITY || ARR_MASK_TEST(stateDone, gi))
                    continue; // state already decomposed at an earlier cubie
                ...
                for (int guard = 0; guard < 4 * MAX_CLUSTER && sp > 0; guard++)
                {
                    ...
                    if (complete)
                    {
                        aMax = max(aMax, agreeAxes);
												//if (pieces < (1u << (N - 1u))) { pAgree[pieces] = agreeAxes; pMask[pieces] = agreeMask; }
                        dHist[agreeAxes] += 1u;
                        pieces += 1u;
                    ...
                    else
                    {
						...
						//uint smallest = 1u << (N - aMax);           // size of the weakest piece
            uint S = bitCount(seenAgree); // self-scaling SYMMETRY = # of distinct piece sizes
						...
						//    uint aa = N - uint(findMSB(blk));       // agreeAxes of this collateral block = N - log2(blk)
						//    collat += float(aa);
						//    s += blk;
						//}
            ...
            for (uint d = N; d >= 1; d--)
            {
                ...
                occ = dHist[d - 1] + par; // dHist[0] stays 0 (no whole-cube block in the residual)
						...
						//local_fA_sum *= float(P2) * Alpha / float((N * (N + 3u) / 2u) * scrambled);   // P3: P2*(1+Beta*(S-1)) -- measured WORSE than P2*S (fat tail)
						...
						//local_fA_sum *= float(aMax * S) / float(N * scrambled);   // aMax*S: ties k+0=k+k=k+k+k+k -> count FLAT within a size level -> backward drift (4 scattered == 8 scattered as a free sideways move). Rejected.
						//local_fA_sum *= float(P2 * S) / float((N * (N + 3u) / 2u) * scrambled);   // P2 * S -- baseline (block-coherence off)
						...
						//local_fA_sum = (float(P2 + N * S) + local_fA_sum)
						//             / float((FLOOR + 2u * N - 1u - uint(findMSB(FLOOR))) * (countActive + 1u));
						...
						//local_fA_sum *= float(2u * P2 * S + b) / float((N * (N + 3u)) * scrambled);   // BC
						...
						//for (uint bkt = 0u; bkt < N * 4u; bkt++) coupCnt[bkt] = 0u;
						//for (uint blk = 0u; blk < pieces; blk++)
						//    for (uint ax = 0u; ax < N; ax++)
						//        if (((blockU[blk] >> ax) & 1u) == 0u)    // axis ax FIXED in this block
						//            coupCnt[ax * 4u + ((blockPos[blk] >> (2u * ax)) & 3u)] += 1u;
						//uint C = 0u;
						//for (uint bkt = 0u; bkt < N * 4u; bkt++) C += coupCnt[bkt] * (coupCnt[bkt] - 1u) / 2u;
						//uint pairs = pieces * (pieces - 1u) / 2u;
						//float Cnorm = float(C) / float(pairs * N + 1u);   // [0,1): shared-coord fraction, sub-unit under *2
						...
						////    ladder = 2u * (Pw + Ew);
						////    structure = min(scrambled, 31u) << (5u * (N - 1u));
						////}
						...
						// FINE MEASURE aligned with the SEEDS: per-cubie L1 toward the SAME target the coherence seeds aim at --
						// targetD (the block orientation) while building, IDENTITY once ONE block remains (collapse it to solved).
						// Replaces the old distance-to-IDENTITY (local_fA_sum), which fought the block build. SELECTION only;
						// the accept stays ladder-based (structure), so the plateau-tolerant sideways is untouched.
            //uint orientRaw = 0u;
            //for (uint oi = 0u; oi < countActive; oi++)
            //{
            //    if (local_active[oi] == IDENTITY)
            //        continue;
            //    orientRaw += (pieces == 1u) ? cubieL1(local_active[oi]) // one block -> IDENTITY (collapse to solved)
							     //                       : matL1(local_active[oi], targetD); // building  -> targetD (the seeds' block orientation)
            //}
            //float orient = float(orientRaw) / float(countActive * uint(MAX_CUBIE_L1) + 1u); // sub-unit (< 1)
            ladder = float(P2 + N * S + 1); // STRUCTURE value -> feeds the fitness (selection), unchanged
            current_step_fitness = local_fA_sum * float(ladder) / float(NORM);
						//current_step_fitness = float((ladder + orient) * FLOOR) / float(NORM * scrambled);
						//local_fA_sum *= (float(2u * P2 * S) + 1 - Cnorm) / float((N * (N + 1u)) * scrambled);   // 2*P2*S + coupling, K=2
						...
						//local_fA_sum *= float(P2 * S + E2) / float((N * (N + 3u) / 2u + 3u) * scrambled);   // +3u = E2 at the 4-singleton floor (2^4)
						...
						//if (pieces == 2u && bitCount(seenAgree) == 1u && aMax < N)
    ...
    Population[specimenID].Ladder = floatBitsToUint(specimen_best_ladder); // STRUCTURE ladder (2*(P2+N*S) etc.) -- the COHERENT accept compares it (Ladder <= ScoreLadder) for sideways-on-equal-structure
}
