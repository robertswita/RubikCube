...
const uint STRIDE = N * N + N + 1u;   // per-instance floats in the Xforms SSBO
...
vec3 project(uint vid, uint inst)
{
    uint vb = vid * N;
    uint mb = inst * STRIDE;
    vec3 p = vec3(0.0);
    for (uint r = 0; r < min(N, 3u); r++)
    {
...
void main()
{
    uint inst = uint(gl_InstanceID + uInstanceBase);
    vec3 p3 = project(uint(gl_VertexID), inst);
    ...
    uint c0 = uint(gl_VertexID) & ~3u;                              // first corner of this quad (= 4 * face)
    vec3 e1 = project(c0 + 1u, inst) - project(c0, inst);    // +u
    vec3 e2 = project(c0 + 3u, inst) - project(c0, inst);    // +v
