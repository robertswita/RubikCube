...
uint matL1(uint M, uint R) {
    uint dist = 0u;
    for (uint r = 0u; r < uint(N); r++) {
        uint fm = (M >> (r * BITS_PER_ROW)) & ((1u << BITS_PER_ROW) - 1u);
        uint fr = (R >> (r * BITS_PER_ROW)) & ((1u << BITS_PER_ROW) - 1u);
        dist += (fm > fr) ? (fm - fr) : (fr - fm);
    }
    return dist;
}
...
uint solvedBrokenCount(uint specimenID, uint prefixLen) {
    ...
    for (uint i = 0u; i < countSolved; i++) {
        ...
        if (cubieL1(mat) != 0u) return 1u;   // zamiast broken++
    }
    return broken;
}
