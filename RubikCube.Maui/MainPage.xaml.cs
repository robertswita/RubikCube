    ...
    private void RunGaLoop(CancellationToken token)
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            if (_ga.HighScore < _highScore)
            {
                ...
                for (int i = 0; i < _ga.Best.BestMovesCount; i++)
                {
    ...
    private void SaveSolution(TRubikGenome solution)
    {
        ...
        if (!_solutions.ContainsKey(code))
        {
            try
            {
                ...
                writer.Write(code);
                writer.Write(solution.BestMovesCount);

                var genes = new List<int>(solution.BestMovesCount);
                for (int i = 0; i < solution.BestMovesCount; i++)
                {
