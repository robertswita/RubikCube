...
void main() {
    ...
    if (specimenID < numSeeds) {
        ...
        //uint rLen = 1u + hash(seed + 101u) % uint(GENES_COUNT / 2 - sLen - 1);    // R length 1..N-1 (as in the mutation)
        //uint pos = sLen;
        //for (uint r = 0u; r < rLen && pos < GENES_COUNT; r++, pos++)                    // R (random free moves)
        //    Population[specimenID].Moves[pos] = FreeMoves[hash(seed + 200u + r) % freeMovesCount];
        ...
        //movesCount = 0;// (hash(seed + 300u) % 2u == 0u) ? 2 * (sLen = rLen) : 2 * sLen + rLen;
        //movesCount = 2 * sLen + rLen;
        //g = pos;
        //movesCount = pos;
