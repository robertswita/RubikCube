...
void main()
{
    ...
    for (int m = 0; m < GENES_COUNT; m++)
    {
        ...
        //}
        if (cut == 0u || cut == uint(m + 1))
        {
            // 2b. EVALUATION -- ONCE, on the final cut state (not per prefix): sum the active-cluster errors
    ...
    //            if (COHERENT != 0u && scrambled > 0u)
            ...
            if (COHERENT != 0u && changed && scrambled > 0u) {   // UNIFIED: decompose EVERY residual for its REAL structure -- an isolated singleton (mono-twist) gets its own EXPENSIVE ladder (deepest structure), so the structure-dominant additive below deters isolating the last cubie. No lie, no even/odd split. Full-cluster aMax==0 trap walled below.
                ...
                local_fA_sum = (float(ladder) + local_fA_sum) / float(NORM);   // OLD: P2 + N*S (depth-weighted)
            ...
            current_step_fitness += float((solvedBroken[uint(m) >> 5u] >> (uint(m) & 31u)) & 1u);
            if (current_step_fitness < specimen_best_fitness)
            {
                specimen_best_fitness = current_step_fitness;
                specimen_best_moves_count = uint(m + 1);                  // the cut IS the answer -- no scan, no best-of-prefixes
